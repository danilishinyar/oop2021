#include "LongNum_static_overload.h"
...
namespace Num_static_overload {
    ...
    /*
     * Конструктор с инициализацией с помощью long
     */
    ...
    /*
     * Конструктор с инициализацией с помощью char*
     */
    ...
    */
    std::istream &operator>>(std::istream &in, LongNum &a) {
        ...
        tmp[i] = '\0';
        LongNum tmp1(tmp);
        a.length = tmp1.length;
        memcpy(a.dights, tmp1.dights, a.length + 1);
        delete[] res;
        delete[] tmp;
        return in;
    }
